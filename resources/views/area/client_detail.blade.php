    ...
    <div class="row">
        <div class="col-lg-3">
            <div class="card" id="card1">
                <div class="card-body">
                    <p style="margin: 0px"><b>{{ $client->customer }}</b></p>
                    <p class="text-muted" style="margin: 0px"><i class="fas fa-map-marker-alt mr-2"></i>{{ empty($client->asal) ? '-' : $client->asal }}</p>
                    <p class="text-muted" style="margin: 0px"><i class="fas fa-phone mr-2"></i>{{ empty($client->handphone) ? '-' : $client->handphone }}</p>
                </div>
            </div>
        </div>
        <div class="col-lg-3">
            <div class="card" id="card2">
                <div class="card-body text-center d-flex">
                    <div class="m-auto">
                        <h2 style="margin: 0px">{{ rupiah($client->total) }}</h2>
        ...
        <div class="col-lg-3">
            <div class="card" id="card3">
                <div class="card-body text-center d-flex">
                    <div class="m-auto">
                        <h2 style="margin: 0px">{{ rupiah($client->pay, true) }}</h2>
                        <p style="margin: 0px">Total Harga</p>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-lg-3">
            <div class="card" id="card4">
                <div class="card-body text-center d-flex">
                    <div class="m-auto">
                        <h2 style="margin: 0px">{{ rupiah($client->margin, true) }}</h2>
...
@push('js')
    <script>
        ...
        var datatable = $("#table-1").DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ url('task/list') }}' + '?tanggal_mulai_filter=' + tanggal_mulai + '&tanggal_akhir_filter=' + tanggal_akhir + '&task_status_filter=' + task_status + '&pay_status_filter=' + pay_status + '&client_id_filter={{ $client->id }}',
            },
            columns: [{
                "data": "DT_RowIndex",
                "name": "DT_RowIndex",
        ...
        function reloadDataTable() {
            ...
            var newUrl = currentUrl + '?tanggal_mulai_filter=' + tanggal_mulai + '&tanggal_akhir_filter=' + tanggal_akhir + '&task_status_filter=' + task_status + '&pay_status_filter=' + pay_status + '&client_id_filter={{ $client->id }}';
